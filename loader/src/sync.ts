...
function tsxBinPath(): string {
  ...
  return path.resolve(LOADER_ROOT, "node_modules", ".bin", bin);
}
...
async function countMissing(): Promise<number> {
  ...
  const { rows } = await pool.query<{ count: number }>(
    "select count(*)::int as count from image_embeddings where embedding is null;"
  );
...
async function main(): Promise<void> {
  ...
  for (;;) {
    ...
    // Embed a batch
    // Allow external EMBED_LIMIT to control batch size; default handled by embed_missing.ts
    // eslint-disable-next-line no-console
    console.log("Running embed phase...");
    await runTsx("src/embed_missing.ts");
