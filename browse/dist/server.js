...
if (!USING_REMOTE_IMAGES) {
    // eslint-disable-next-line no-console
    console.error("IMAGE_BASE_URL is not set; serving images from local /images instead of remote bucket. Set IMAGE_BASE_URL to your R2 public base URL.");
}
let pool = null;
if (DATABASE_URL) {
...
function resolveImageUrl(fileName) {
    ...
    if (!warnedLocalImages) {
        // eslint-disable-next-line no-console
        console.error("Serving images from local filesystem. Configure IMAGE_BASE_URL to use the remote bucket.");
        warnedLocalImages = true;
    }
    return `/images/${encodeURIComponent(fileName)}`;
}
function listLocalImages(limit = 60) {
    try {
...
async function listDbImages(limit = 60) {
    if (!pool)
        return [];
    const { rows } = await pool.query(`SELECT file_name
...
function toVectorParam(embedding) {
    if (typeof embedding === "string")
        return embedding; // expected like "[0.1,0.2,...]"
...
function renderTemplate(images, query) {
...
app.get("/stats", async (_req, res) => {
    try {
        ...
        const { rows: totalRows } = await pool.query(`SELECT count(*)::int AS count FROM image_embeddings;`);
        const { rows: encodedRows } = await pool.query(`SELECT count(*)::int AS count FROM image_embeddings WHERE embedding IS NOT NULL;`);
...
app.get("/", async (req, res) => {
    ...
    try {
        if (q) {
            ...
            if (!pool)
                throw new Error("Database not configured");
            const { rows } = await pool.query(`SELECT file_name, embedding <#> $1::vector AS distance
         FROM image_embeddings
         WHERE embedding IS NOT NULL
...
app.get("/neighbors/:file_name", async (req, res) => {
    const fileName = req.params.file_name;
    try {
        if (!pool)
            throw new Error("Database not configured");
        // Fetch embedding for the selected image as text for reuse
        const { rows: embRows } = await pool.query(`SELECT embedding::text AS embedding_text
       FROM image_embeddings
       ...
       ORDER BY distance ASC
       LIMIT 30;`, [embeddingText, fileName]);
