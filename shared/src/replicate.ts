...
const DEFAULT_IMAGE_MODEL: ReplicateModelId =
  (process.env.REPLICATE_IMAGE_MODEL as ReplicateModelId) ||
  ("krthr/clip-embeddings:1c0371070cb827ec3c7f2f28adcdde54b50dcd239aa6faea0bc98b174ef03fb4" as ReplicateModelId);
const DEFAULT_TEXT_MODEL: ReplicateModelId =
  (process.env.REPLICATE_TEXT_MODEL as ReplicateModelId) ||
  ("krthr/clip-embeddings:1c0371070cb827ec3c7f2f28adcdde54b50dcd239aa6faea0bc98b174ef03fb4" as ReplicateModelId);
...
function parseEmbeddingResult(result: unknown, context: string): number[] {
  if (Array.isArray(result)) {
    const embedding = result.map((x) => Number(x));
    if (EXPECTED_VECTOR_DIM && embedding.length !== EXPECTED_VECTOR_DIM) {
      throw new Error(`${context}: Embedding dimension mismatch: got ${embedding.length}, expected ${EXPECTED_VECTOR_DIM}`);
    }
    return embedding;
  }
  if (result && typeof result === "object" && Array.isArray((result as any).embedding)) {
    ...
    if (EXPECTED_VECTOR_DIM && vec.length !== EXPECTED_VECTOR_DIM) {
      throw new Error(`${context}: Embedding dimension mismatch: got ${vec.length}, expected ${EXPECTED_VECTOR_DIM}`);
    }
    return vec;
  }
  throw new Error(`${context}: Unexpected embedding result from Replicate (expected number[] or {embedding:number[]})`);
}
...
export async function getImageEmbedding(imageUrl: string): Promise<number[]> {
  ...
  const result: unknown = await client.run(modelId, { input });
  return parseEmbeddingResult(result, "getImageEmbedding");
}

export async function getTextEmbedding(query: string): Promise<number[]> {
  const modelId = DEFAULT_TEXT_MODEL;
  const client = getReplicateClient();
  const input: Record<string, unknown> = {};
  input[TEXT_INPUT_KEY] = query;
  const result: unknown = await client.run(modelId, { input });
  return parseEmbeddingResult(result, "getTextEmbedding");
}
